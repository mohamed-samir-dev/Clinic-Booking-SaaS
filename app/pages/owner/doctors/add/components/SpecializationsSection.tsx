...
export default function SpecializationsSection({ specializations, onUpdate }: SpecializationsSectionProps) {
  ...
  return (
    <div className="bg-gray-700/50 p-5 rounded-xl border border-gray-600">
      <div className="flex items-center gap-3 mb-3">
        ...
        <h3 className="text-lg font-bold text-white">التخصصات الفرعية - Specializations</h3>
      </div>
      ...
      <div className="bg-gray-800 p-4 rounded-lg border-2 border-gray-600 space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-semibold text-gray-300 mb-2">التخصص (إنجليزي) - Specialization (English)</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-300 mb-2">التخصص (عربي) - Specialization (Arabic)</label>
            <input
              type="text"
              value={newSpecialization.ar}
              onChange={(e) => setNewSpecialization({ ...newSpecialization, ar: e.target.value })}
              className="w-full px-4 py-2 bg-gray-700 border-2 border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 text-white"
              placeholder="مثال: أمراض قلب الأطفال"
            />
          </div>
        </div>
        <button
