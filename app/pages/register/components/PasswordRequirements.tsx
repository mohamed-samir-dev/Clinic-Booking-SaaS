...
export const PasswordRequirements = ({ password }: PasswordRequirementsProps) => {
  ...
  const t = locale === 'ar' ? messagesAr.auth.register.passwordRequirements : messages.auth.register.passwordRequirements;
  ...
  <div className={`mt-2 sm:mt-3 border rounded-lg p-2.5 sm:p-3 ${theme === 'dark' ? 'bg-blue-900/20 border-blue-800' : 'bg-blue-50 border-blue-200'}`}>
    <p className={`text-xs font-semibold mb-1.5 sm:mb-2 ${theme === 'dark' ? 'text-blue-300' : 'text-blue-900'}`}>{t.title}</p>
    <ul className="space-y-0.5 sm:space-y-1">
      ...
      <li className={`text-xs flex items-center gap-1.5 sm:gap-2 ${/[A-Z]/.test(password) ? 'text-teal-600' : theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
