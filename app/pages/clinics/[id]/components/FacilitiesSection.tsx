...
export default function FacilitiesSection({ facilities, theme }: FacilitiesSectionProps) {
  ...
  return (
    <div className={`rounded-xl shadow p-4 sm:p-6 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
      <h2 className={`text-xl sm:text-2xl font-bold mb-3 sm:mb-4 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Facilities & Services</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
        {facilities.map((facility, index) => (
          <div key={index} className={`flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-lg ${theme === 'dark' ? 'bg-teal-900/30' : 'bg-teal-50'}`}>
            ...
            <p className={`font-semibold text-sm sm:text-base ${theme === 'dark' ? 'text-gray-200' : 'text-gray-900'}`}>{facility.name.en}</p>
          </div>
        ))}
      </div>
    </div>
