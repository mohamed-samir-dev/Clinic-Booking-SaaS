  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className={`rounded-xl shadow p-4 sm:p-6 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
        <h3 className={`text-lg sm:text-xl font-bold mb-3 sm:mb-4 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{t('contactInformation')}</h3>
        <div className="space-y-3 sm:space-y-4">
          {(clinic.address?.[locale] || clinic.address?.en) && (
            <div className="flex gap-2 sm:gap-3">
              <MapPin className="text-teal-600 shrink-0 mt-1" size={18} />
              <div className="min-w-0">
                <p className={`text-xs sm:text-sm mb-1 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>{t('address')}</p>
                <p className={`font-medium text-sm sm:text-base ${theme === 'dark' ? 'text-gray-200' : 'text-gray-700'}`}>{clinic.address[locale] || clinic.address.en}</p>
              </div>
            </div>
          )}
          ...
          {clinic.phone && (
            <a href={`tel:${clinic.phone}`} className={`flex gap-2 sm:gap-3 p-2 rounded-lg transition ${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
              ...
              <div className="min-w-0">
                <p className={`text-xs sm:text-sm mb-1 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>{t('phone')}</p>
          ...
          {clinic.email && (
            <a href={`mailto:${clinic.email}`} className={`flex gap-2 sm:gap-3 p-2 rounded-lg transition ${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>
              ...
              <div className="min-w-0">
                <p className={`text-xs sm:text-sm mb-1 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>{t('email')}</p>
      ...
      {clinic.capacity && (
        <div className={`rounded-xl shadow p-4 sm:p-6 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
          <h3 className={`text-lg sm:text-xl font-bold mb-3 sm:mb-4 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{t('capacity')}</h3>
            ...
            {clinic.capacity.rooms > 0 && (
              <div className={`flex items-center justify-between p-2 sm:p-3 rounded-lg ${theme === 'dark' ? 'bg-teal-900/30' : 'bg-teal-50'}`}>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-sm sm:text-base ${theme === 'dark' ? 'text-gray-200' : 'text-gray-700'}`}>{t('rooms')}</span>
                </div>
            ...
            {clinic.capacity.patientsPerDay > 0 && (
              <div className={`flex items-center justify-between p-2 sm:p-3 rounded-lg ${theme === 'dark' ? 'bg-teal-900/30' : 'bg-teal-50'}`}>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-sm sm:text-base ${theme === 'dark' ? 'text-gray-200' : 'text-gray-700'}`}>{t('patientsPerDay')}</span>
                </div>
      ...
      {clinic.socialMedia && (
        <div className={`rounded-xl shadow p-4 sm:p-6 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
          <h3 className={`text-lg sm:text-xl font-bold mb-3 sm:mb-4 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{t('followUs')}</h3>
