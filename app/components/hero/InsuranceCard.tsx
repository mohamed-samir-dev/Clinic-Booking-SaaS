      ...
      <div className={`backdrop-blur-sm p-3 rounded-lg md:rounded-xl shadow-lg h-full flex items-center lg:w-80 xl:w-[400px] ${theme === 'dark' ? 'bg-gray-800/95' : 'bg-white/95'}`}>
        <div className="flex items-center gap-2 md:gap-3 w-full">
          ...
          <div className="flex-1">
            <p className={`text-xs md:text-sm font-semibold ${theme === 'dark' ? 'text-gray-200' : 'text-gray-900'}`}>Insurance Accepted:</p>
            <h2 className={`text-sm md:text-base lg:text-lg xl:text-[20px] font-bold mt-0.5 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>Major plan & Network</h2>
          </div>
        </div>
      </div>
    </div>
